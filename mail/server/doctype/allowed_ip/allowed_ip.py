...
class AllowedIP(Document):
	...
	def _get(self) -> None:
		...
		backend_api = get_management_backend_adapter()
		response = backend_api.settings_group("server.allowed-ip")

		data = (response.data or {}).get("data", {})
		items = data.get("items", [])
		allowed_ip = next((item for item in items if item.get("_id") == ip_address), None)
		if not allowed_ip:
			frappe.throw(_("Allowed IP {0} not found.").format(ip_address))
	...
	def _get_all(cluster: str, page: int = 1, limit: int = 10, text: str | None = None) -> list:
		...
		return [AllowedIP._format(item, cluster) for item in page_items]
